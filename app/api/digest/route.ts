    ...
    try {
      const completion = await groq.chat.completions.create({
        messages: [
          {
            role: 'system',
            content: `Ти досвідчений аналітик новин (рівень політичного або медійного оглядача). Твоє завдання — дати короткий професійно-аналітичний висновок за заголовками, а не просто їх перелічити.

Формат відповіді українською (2–4 речення):
1. Початок обов'язково: "Головне:" — одне речення про те, що зараз у фокусі (ключові теми/події).
2. Далі — аналітичний висновок: що це означає, чому важливо, який тренд або наслідки. Стиль як у коментаря експерта: конкретно, без води, з оцінкою.

Не переказуй заголовки. Давай саме висновок і контекст — що об'єднує ці події і що з цього випливає.`,
          },
          {
            role: 'user',
            content: `Заголовки новин:\n${list}\n\nДай короткий професійно-аналітичний висновок (початок "Головне:", далі 1–3 речення з оцінкою та контекстом).`,
          },
        ],
